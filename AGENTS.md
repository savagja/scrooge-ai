...
When I (the coding agent) need to deploy code changes to the Pi, use the deploy scripts at `deploy/`. The process:
...
**Pre-deploy checklist** (run these checks before deploying):
...
## 🚀 Deployment Target — Data is on the Pi
...
### ⚠️ IMPORTANT: Data Source Convention
